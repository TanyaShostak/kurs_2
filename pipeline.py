...
class ProbingPipeline:
    ...
    def evaluate(
        ...
        return epoch_loss, epoch_metric_score, epoch_predictions, epoch_true_labels
    ...
    def run(
        ...
        for layer in probing_iter_range:
            ...
            for epoch in range(train_epochs):
                ...
                epoch_val_loss, epoch_val_score, epoch_val_predictions, epoch_val_true_labels = self.evaluate(
            ...
            _, epoch_test_score, epoch_test_predictions, epoch_test_true_labels = self.evaluate(
                probing_dataloaders["te"], layer, save_checkpoints
            )
            print(epoch_test_predictions)
            #np.save(pred, "file.txt")
            #log_info["results"]["predictions"].add(layer, epoch_test_predictions)
            #log_info["results"]["true_labels"].add(layer, epoch_test_true_labels)

            for m in self.metric_names:
                log_info["results"]["test_score"][m].add(layer, epoch_test_score[m])
        print(epoch_test_true_labels)
